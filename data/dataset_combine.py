...
class VideoSameSizeDataset(data.Dataset):
    def __init__(self, opt):
        ...
        self.data_info = {'path_LQ': [], 'path_GT': [], 'folder': [], 'idx': [], 'GT_muti': []}
        self.dataset_type_list = self.opt['datasetlist']
        ...
        for mm in range(len(subfolders_LQ_origin)):
            if self.opt['phase'] == 'train':
                ...
                for c_databasename in self.opt['datasetlist']:
                    if c_databasename in name:
    ...
    def __getitem__(self, index):
        ...
        GT_max_idx = self.data_info['GT_muti'][index]
        ...
        image_pair = {'img_LQ_path': img_LQ_path, 'img_GT_path': img_GT_path}
        if self.opt['phase'] == 'train':
            img_pair_data = util.read_img_crop(image_pair, self.opt['train_size'])
            img_LQ = img_pair_data['img_LQ']
            img_GT = img_pair_data['img_GT']
            ...
            img_LQ = util.read_img_seq2(img_LQ_path, self.opt['val_size'])
            img_GT = util.read_img_seq2(img_GT_path, self.opt['val_size'])
            img_LQ = img_LQ[0]
            img_GT = img_GT[0]
        ...
        img_LQ_path_basename = os.path.basename(img_LQ_path[0])
        ...
        return {
            ...
            'LQ_basename': img_LQ_path_basename
        }
