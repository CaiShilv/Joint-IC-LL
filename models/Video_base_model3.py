...
class VideoBaseModel(BaseModel):
    ...
    def feed_data_runtest(self, data, need_GT=True):
        # print(data['LQs'].shape)
        self.var_L, self.cor_var_L = pad.pad(data['LQs'].to(self.device), 64)
        self.nf, self.cor_nf = pad.pad(data['nf'].to(self.device), 64)
        if need_GT:
            self.real_H, self.cor_real_H = pad.pad(data['GT'].to(self.device), 64)

    def set_params_lr_zero(self):
        # fix normal module
        self.optimizers[0].param_groups[0]['lr'] = 0

    def optimize_parameters(self, step, compress=True):
        """
        ************************************************************
        input: step
        output:
        ...
        ************************************************************
        ...
        # input_x = (self.var_L,None,mask,True, compress)
        # flops, params = profile(self.netG, inputs=input_x)
        # flops_m = flops/1024./1024./1024.
        # params_m = params/1024./1024.
        # print("**************************** test *************************************")
        # print(f"flops:{flops_m}G, params:{params_m}M")
        # n_parameters = sum(p.numel() for p in self.netG.parameters() if p.requires_grad) / 1024. / 1024.
        # print(f'number of params: {n_parameters:.6f}M flops')  # 数量代表网络权重浮点计算次数 flops
        # **************************** test *************************************

        net_output = self.netG(self.var_L, gt=None, mask=mask, ll_enhance=True, compress=compress)  # x, gt=None, mask, ll_enhance
        loss = self.loss(net_output, self.real_H, compress)
        # out = {'loss': 0,
        #        'weight_fea_loss': 0,
        #        'weight_dis_loss': 0,
        #        'fea_loss': 0,
        ...
        self.log_dict['fea_loss'] = loss['fea_loss']
        self.log_dict['dis_loss'] = loss['dis_loss']
        self.log_dict['weight_fea_loss'] = loss['weight_fea_loss']
        self.log_dict['weight_dis_loss'] = loss['weight_dis_loss']
        self.log_dict['bpp_loss'] = loss['bpp_loss']
        self.log_dict['weight_bpp_loss'] = loss['weight_bpp_loss']
        self.log_dict['weight_vgg_loss'] = loss['weight_vgg_loss']
        ...
        if not optimizer_flag:
            message = '[Warning]: network parameters are not optimized due to train loss: {:.4f},' \
                      ' fea_loss: {:.4f}, ' \
                      'dis_loss: {:.4f}, ' \
                      'weight_fea_loss:{:.4f}, ' \
                      'weight_dis_loss:{:.4f}, ' \
                      'bpp_loss:{:.4f}, weight_vgg_loss:{:.4f}'.format(loss['loss'].item(),
                                                      loss['fea_loss'].item(),
                                                      loss['dis_loss'].item(),
                                                      loss['weight_fea_loss'].item(),
                                                      loss['weight_dis_loss'].item(),
                                                      loss['bpp_loss'],
                                                      loss['weight_vgg_loss'].item())
            logger.info(message)
    ...
    def fullmodel_val(self, compress=True):
        self.netG.eval()
        self.netG.module.update()
        with torch.no_grad():
            dark = self.var_L
            dark = dark[:, 0:1, :, :] * 0.299 + dark[:, 1:2, :, :] * 0.587 + dark[:, 2:3, :, :] * 0.114
    ...
    def fullmodel_test(self):
        ...
        with torch.no_grad():
            ...
            x_hat = self.netG.module.decompress(bitstreams['strings'], shape_bitstream)
            self.fake_H = x_hat['x_hat']
            num_pixels = dark.size(0) * (dark.size(2)-self.cor_var_L[2]-self.cor_var_L[3]) * (dark.size(3)-self.cor_var_L[0]-self.cor_var_L[1])
            extra_bytes = 56
            bpp = (sum(len(s[0]) for s in bitstreams["strings"]) + extra_bytes) * 8.0 / num_pixels
    ...
    def fullmodel_test_estimate(self):
        ...
        with torch.no_grad():
            ...
            mask = mask.float()
            # self.fake_H = self.netG(self.var_L, mask)
            net_output = self.netG(self.var_L, gt=None, mask=mask, ll_enhance=True, compress=True)  # x, gt=None, mask, ll_enhance
            self.fake_H = net_output['x_hat']
            num_pixels = dark.size(0) * (dark.size(2)-self.cor_var_L[2]-self.cor_var_L[3]) * (dark.size(3)-self.cor_var_L[0]-self.cor_var_L[1])
